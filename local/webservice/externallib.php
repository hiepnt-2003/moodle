...
class local_webservice_external extends external_api {
        ...
        try {
            ...
            if (!$source_course) {
                return [
                    'status' => 'error',
                    'id' => 0,
                    'message' => get_string('error_coursenotfound', 'local_webservice', $params['shortname_clone'])
                ];
            }
            ...
            if ($DB->record_exists('course', ['shortname' => $params['shortname']])) {
                return [
                    'status' => 'error', 
                    'id' => 0,
                    'message' => get_string('error_shortnameinuse', 'local_webservice', $params['shortname'])
                ];
            }
            ...
            return [
                'status' => 'success',
                'id' => $new_course->id,
                'message' => get_string('success_coursecloned', 'local_webservice')
            ];
    ...
    private static function validate_clone_parameters($params) {
        ...
        if (!is_numeric($params['startdate']) || !is_numeric($params['enddate'])) {
            return [
                'success' => false,
                'message' => get_string('error_invaliddate', 'local_webservice')
            ];
        }
        ...
        if ($params['enddate'] <= $params['startdate']) {
            return [
                'success' => false,
                'message' => get_string('error_enddatebeforestartdate', 'local_webservice')
            ];
        }
        ...
        if (empty(trim($params['fullname'])) || empty(trim($params['shortname']))) {
            return [
                'success' => false,
                'message' => get_string('error_invalidparameter', 'local_webservice', 'fullname or shortname')
            ];
        }
    ...
    private static function create_course_backup($courseid) {
        ...
        try {
            ...
            return [
                ...
                'message' => get_string('backup_completed', 'local_webservice')
            ];

        } catch (Exception $e) {
            return [
                'success' => false,
                'message' => get_string('error_backupfailed', 'local_webservice') . ': ' . $e->getMessage()
            ];
        }
    }
    ...
    private static function restore_course_backup($backup_path, $courseid) {
        ...
        try {
            ...
            if ($rc->execute_precheck()) {
                ...
                return [
                    'success' => true,
                    'message' => get_string('restore_completed', 'local_webservice')
                ];
            } else {
                $rc->destroy();
                return [
                    'success' => false,
                    'message' => get_string('error_restorefailed', 'local_webservice') . ': Precheck failed'
                ];
            }
            ...
            return [
                'success' => false,
                'message' => get_string('error_restorefailed', 'local_webservice') . ': ' . $e->getMessage()
            ];
        }
    }
